-- ============================================================
-- SESAME — Migration sûre (données existantes préservées)
-- À coller dans Supabase SQL Editor et exécuter
-- ✅ Ne supprime rien, ne détruit aucune donnée
-- ============================================================

-- ─── 1. EXTENSION ────────────────────────────────────────────
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- ─── 2. TYPES ENUM (ignorés si déjà existants) ───────────────
DO $$ BEGIN CREATE TYPE utilisateur_type AS ENUM ('ambassadeur', 'chauffeur', 'admin'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE utilisateur_statut AS ENUM ('actif', 'suspendu', 'blackliste'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE theme_type AS ENUM ('nocturne', 'clair', 'auto'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE langue_type AS ENUM ('fr', 'en', 'it', 'es'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE type_ambassadeur AS ENUM ('physique', 'moral'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE amb_niveau AS ENUM ('starter', 'pro', 'elite', 'black'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE sous_compte_statut AS ENUM ('actif', 'suspendu'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE vehicule_type AS ENUM ('berline', 'van'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE doc_type AS ENUM ('carte_identite','carte_vtc','revtc','kbis','permis','rir','rc_pro','rc_circulation','carte_grise','certificat_medical','photo_profil'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE document_statut AS ENUM ('en_attente', 'valide', 'refuse', 'expire'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE course_statut AS ENUM ('recherche','acceptee','en_route','code_valide','en_cours','terminee','annulee'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE course_type AS ENUM ('immediate', 'reservation'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE annule_par_type AS ENUM ('ambassadeur', 'chauffeur', 'admin'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE option_paiement_type AS ENUM ('a', 'b', 'c'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE fournisseur_statut AS ENUM ('actif', 'inactif', 'suspendu', 'en_configuration'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE offre_statut AS ENUM ('en_ligne', 'hors_ligne'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE echange_statut AS ENUM ('en_attente_admin','valide','refuse','utilise','expire'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE blacklist_type_utilisateur AS ENUM ('ambassadeur', 'chauffeur'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE points_type AS ENUM ('gain','depense','compensation','sanction','parrainage','expiration'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE sanctions_statut AS ENUM ('en_attente', 'execute'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ─── 3. TABLES (créées seulement si absentes) ─────────────────
...
-- ─── 4. NOUVELLES COLONNES (ignorées si déjà présentes) ───────
-- utilisateurs
ALTER TABLE utilisateurs ADD COLUMN IF NOT EXISTS date_naissance      date;
ALTER TABLE utilisateurs ADD COLUMN IF NOT EXISTS lieu_naissance      varchar(100);
ALTER TABLE utilisateurs ADD COLUMN IF NOT EXISTS pays_naissance      varchar(100);
ALTER TABLE utilisateurs ADD COLUMN IF NOT EXISTS reset_code          varchar(6);
ALTER TABLE utilisateurs ADD COLUMN IF NOT EXISTS reset_code_expires_at timestamptz;

-- ambassadeurs
ALTER TABLE ambassadeurs ADD COLUMN IF NOT EXISTS etablissement           varchar(200);
ALTER TABLE ambassadeurs ADD COLUMN IF NOT EXISTS metier                  varchar(100);
ALTER TABLE ambassadeurs ADD COLUMN IF NOT EXISTS siret                   varchar(14);
ALTER TABLE ambassadeurs ADD COLUMN IF NOT EXISTS iban                    varchar(50);
ALTER TABLE ambassadeurs ADD COLUMN IF NOT EXISTS responsable_legal_nom   varchar(200);
...
-- chauffeurs
ALTER TABLE chauffeurs ADD COLUMN IF NOT EXISTS taux_commission_override numeric(5,2);
ALTER TABLE chauffeurs ADD COLUMN IF NOT EXISTS iban                     varchar(50);
...
ALTER TABLE courses ADD COLUMN IF NOT EXISTS taux_commission_applique decimal(5,2);
ALTER TABLE courses ADD COLUMN IF NOT EXISTS code_validation          varchar(4);
ALTER TABLE courses ADD COLUMN IF NOT EXISTS code_valide_at           timestamptz;
ALTER TABLE courses ADD COLUMN IF NOT EXISTS points_attribues         integer DEFAULT 0;
...
ALTER TABLE echanges ADD COLUMN IF NOT EXISTS fournisseur_id      uuid REFERENCES fournisseurs(id);
ALTER TABLE echanges ADD COLUMN IF NOT EXISTS token_qr            varchar(255);
ALTER TABLE echanges ADD COLUMN IF NOT EXISTS remis_at            timestamptz;
...
-- fournisseurs
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS prest_prenom           varchar(100);
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS prest_nom              varchar(100);
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS prest_telephone        varchar(20);
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS prest_email            varchar(255);
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS prest_adresse          text;
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS prest_cp               varchar(10);
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS prest_ville            varchar(100);
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS memes_coordonnees      boolean NOT NULL DEFAULT false;
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS nb_tentatives_echouees integer NOT NULL DEFAULT 0;
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS bloque                 boolean NOT NULL DEFAULT false;
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS contrat_signe          boolean NOT NULL DEFAULT false;
ALTER TABLE fournisseurs ADD COLUMN IF NOT EXISTS contrat_signe_at       timestamptz;
...
INSERT INTO parametres_systeme (cle, valeur, description) VALUES
    ...
    ('berline_prix_km',                 '2.00',  NULL),
    ('van_forfait',                     '12.00', NULL),
    ('van_seuil_km',                    '6',     NULL),
    ('van_prix_km',                     '3.00',  NULL),
    ('commission_ambassadeur_moral_pct','10',    'Commission en % pour Ambassadeur Moral')
